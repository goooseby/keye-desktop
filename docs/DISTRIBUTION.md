...
当前开发版及本地测试安装包已构建成功，尚未完成安装、卸载和应用内更新的端到端验收。分发目标是一个 `Keye_x.y.z_x64-setup.exe`：双击后进入 Windows 安装向导，可选择安装目录。配置采用 Tauri 2 的 NSIS `currentUser` 模式，默认安装给当前用户，不要求管理员权限；选择其他位置时以 Windows 对目标目录的实际写入权限为准。应用运行时不依赖安装路径可写。
